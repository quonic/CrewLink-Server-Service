...
pushd $base
if [ "$(journalctl -u crewlink-\*.service --since=-1h | awk '{print ($0 ~ /\"GET \/[0-9]+.[0-9]+.[0-9]+.yml HTTP\/1.1\" 404/)?1:0}' | tail -n 1)" -eq 1 ] || [ "$(git rev-list HEAD...origin/master --count)" -gt "0" ]; then
  ...
  if [[ "$(journalctl -u crewlink-\*.service --since=-1h | tail -n 4)" =~ .*"CrewLink Server started".* ]]; then
